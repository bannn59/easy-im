  ...
  return (
    <section className="page">
      ...
      <p className="page__lead">
        Create a conversation with another registered user email, or open one from the list.
        Messaging arrives in the next milestone.
      </p>
    </section>
  );
}
...
export function ConversationRoom() {
  ...
  return (
    <section className="page">
      ...
      <div className="panel" aria-label="Members">
        {(conv.members ?? []).map((m) => (
          <div className="panel__row" key={m.id}>
            <span className="panel__key">Member</span>
            <span className="panel__val">{m.email}</span>
          </div>
        ))}
      </div>

      <p className="page__body">
        Message composer will land in T4. This room is an empty, ACL-checked shell.
      </p>
    </section>
  );
}
